...
import '@/app/globals.css'; // Ensure fonts are available
// Dynamically load fonts if possible or rely on Head in layout. 
// For now, let's assume layout handles it or we add a style tag.
...
// Rewriting slightly to ensure Document wraps everything correctly.
export function PdfViewer({ pdfData, currentPage, zoom, annotations, highlights, addAnnotation, addHighlight, currentUser, onDocumentLoadSuccess, isDualPage, numPages, interactionMode, highlightColor }: PdfViewerProps) {
  ...
  // Just for the demo of the effect, let's create a functional book wrapper.
  // We need to ensure the Document component is the parent.

  return (
    <div className="w-full h-full overflow-hidden flex justify-center items-center bg-muted/50 p-4">
      <Document
        file={pdfData}
      ...
      >
        <div className="book-container">
          <div className="book">
            {/* Single Page View */}
            {!isDualPage && (
              <div className="book__page book__page--front" style={{ position: 'relative', transform: 'none', left: 'auto' }}>
                <div className="book-content">
                  ...
                  />
                  <div className="absolute bottom-4 left-0 w-full text-center text-xs text-muted-foreground">{currentPage}</div>
                </div>
              </div>
            )}

            {/* Dual Page View - Left Page */}
            {isDualPage && currentPage > 1 && (
              <div className="book__page book__page--back" style={{ transform: 'rotateY(0deg)', left: '-100%' }}>
                <div className="book-content">
                  <SinglePageView
                    ...
                    highlightColor={highlightColor}
                  />
                  <div className="absolute bottom-4 left-0 w-full text-center text-xs text-muted-foreground">{currentPage - 1}</div>
                </div>
              </div>
            )}

            {/* Dual Page View - Right Page */}
